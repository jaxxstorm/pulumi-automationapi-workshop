...
class CertificateSpecArgs:
    def __init__(__self__, *,
        ...
        :param pulumi.Input[Sequence[pulumi.Input[str]]] email_addresses: EmailAddresses is a list of email subjectAltNames to be set on the Certificate.
        ...
        :param pulumi.Input[Sequence[pulumi.Input[str]]] uris: URIs is a list of URI subjectAltNames to be set on the Certificate.
...
class CertificateSpecPrivateKeyArgs:
    def __init__(__self__, *,
        ...
        """
        Options to control private keys used for the Certificate.
        :param pulumi.Input[str] algorithm: Algorithm is the private key algorithm of the corresponding private key for this certificate. If provided, allowed values are either `RSA` or `ECDSA` If `algorithm` is specified and `size` is not provided, key size of 256 will be used for `ECDSA` key algorithm and key size of 2048 will be used for `RSA` key algorithm.
        :param pulumi.Input[str] encoding: The private key cryptography standards (PKCS) encoding for this certificate's private key to be encoded in. If provided, allowed values are `PKCS1` and `PKCS8` standing for PKCS#1 and PKCS#8, respectively. Defaults to `PKCS1` if not specified.
